...
1. Gồm 5 câu hỏi.
2. KHÔNG đưa đáp án.
3. KHÔNG sử dụng định dạng LaTeX như $$, \[, \] hay \( \).
4. BẮT BUỘC TRẢ VỀ ĐÚNG ĐỊNH DẠNG JSON SAU, không có markdown (```json):
{{
  "type": "exam",
  "topic": "{topic}",
  "questions": [
    ...
    {{
      "id": 2,
      "type": "fill_blank",
      "question": "Nội dung câu điền khuyết?"
    }}
  ]
}}
"""
        try:
            response = await self.llm_service.generate_response(
                system_prompt="You are a Math teacher. Output ONLY valid JSON representing the exam. DO NOT wrap in ```json. Use plain text for math.",
                user_prompt=prompt
            )
            response = response.replace("```json", "").replace("```", "").strip()
            data = json.loads(response) # Verify it is valid JSON
            return f"[INTERACTIVE_TASK] {json.dumps(data, ensure_ascii=False)}"
        except Exception as e:
            ...
            fallback = {
                ...
                "topic": topic,
                "questions": [
                    {"id": 1, "type": "fill_blank", "question": "Tính: 2 + 3 = ?"},
                    {"id": 2, "type": "fill_blank", "question": "Một hình vuông có cạnh 4cm. Tính chu vi."},
                    {"id": 3, "type": "radio", "question": "Khẳng định nào đúng?", "options": ["10 - 3 = 6", "10 - 3 = 7", "10 - 3 = 8"]},
                    {"id": 4, "type": "fill_blank", "question": "Giải x + 2 = 5"},
                    {"id": 5, "type": "radio", "question": "3 * 2 = ?", "options": ["5", "6", "7"]}
                ]
            }
            return f"[INTERACTIVE_TASK] {json.dumps(fallback, ensure_ascii=False)}"
    ...
    async def generate_theory(self, student_id: int, topic: str) -> str:
        ...
        prompt = f"""Bạn là một giáo viên Toán. Hãy soạn **BÀI HỌC LÝ THUYẾT** đầy đủ cho học sinh.
...
2. KHÔNG sử dụng định dạng LaTeX như $$, \\[, \\], \\( \\).
3. BẮT BUỘC phải có ĐỦ 3 phần sections theo thứ tự:
   - "Khái niệm / Định nghĩa": Giải thích rõ ràng, đơn giản.
   - "Công thức / Quy tắc": Liệt kê công thức hoặc quy tắc quan trọng cần nhớ.
   - "Ví dụ minh họa": Đưa ra 2-3 ví dụ cụ thể, có lời giải từng bước để học sinh dễ hiểu.
4. BẮT BUỘC TRẢ VỀ ĐÚNG ĐỊNH DẠNG JSON SAU, không có markdown (```json):
{{
  "type": "theory",
  "topic": "{topic}",
  "sections": [
    ...
    {{
      "title": "Công thức / Quy tắc",
      "content": "Công thức hoặc quy tắc cần nhớ..."
    }},
    {{
...
"""
        try:
            response = await self.llm_service.generate_response(
                system_prompt="You are a Math teacher. Output ONLY valid JSON representing the theory lesson. DO NOT wrap in ```json.",
                user_prompt=prompt
            )
            response = response.replace("```json", "").replace("```", "").strip()
            ...
            fallback = {
                ...
                "topic": topic,
                "sections": [
                    {"title": "Khái niệm / Định nghĩa", "content": "Phép cộng là phép tính gộp hai hay nhiều số với nhau để tìm tổng.\nPhép trừ là phép tính ngược lại của phép cộng, dùng để tìm hiệu của hai số."},
                    {"title": "Công thức / Quy tắc", "content": "Phép cộng: a + b = c (c là tổng)\nPhép trừ: a - b = d (d là hiệu)\n\nLưu ý: Kết quả phép cộng luôn lớn hơn hoặc bằng mỗi số hạng."},
                    {"title": "Ví dụ minh họa", "content": "Ví dụ 1: Tính 123 + 456\nGiải: 123 + 456 = 579\n\nVí dụ 2: Tính 800 - 345\nGiải: 800 - 345 = 455\n\nVí dụ 3: Một cửa hàng có 500 quyển sách, bán đi 123 quyển. Hỏi còn lại bao nhiêu quyển?\nGiải: 500 - 123 = 377 (quyển)"}
                ]
            }
            return f"[INTERACTIVE_TASK] {json.dumps(fallback, ensure_ascii=False)}"
    ...
    async def generate_free_practice(self, student_id: int, topic: str) -> str:
        ...
        prompt = f"""Bạn là một gia sư Toán thân thiện. Hãy tạo **BÀI TẬP TỰ DO Luyện Tập** cho học sinh.
...
Yêu cầu:
1. Gồm 3-5 câu hỏi thú vị.
2. KHÔNG đưa đáp án trước.
3. KHÔNG sử dụng định dạng LaTeX.
4. BẮT BUỘC TRẢ VỀ ĐÚNG ĐỊNH DẠNG JSON SAU, không có markdown:
{{
  "type": "practice",
  "topic": "Luyện tập tự do: {topic}",
  "questions": [
    {{
      "id": 1,
      "type": "radio",
      "question": "Câu trắc nghiệm?",
      "options": ["A", "B", "C", "D"]
    }},
    {{
      "id": 2,
      "type": "fill_blank",
      "question": "Câu điền khuyết?"
    }}
  ]
}}
"""
        try:
            response = await self.llm_service.generate_response(
                system_prompt="You are a Math tutor. Output ONLY valid JSON representing the practice exercises. DO NOT wrap in ```json.",
                user_prompt=prompt
            )
            response = response.replace("```json", "").replace("```", "").strip()
            data = json.loads(response)
            ...
            fallback = {
                "type": "practice",
                "topic": f"Luyện tập tự do: {topic}",
                "questions": [
                    {"id": 1, "type": "fill_blank", "question": "Tính: 10 + 5 = ?"},
                    {"id": 2, "type": "fill_blank", "question": "Tính: 20 - 5 = ?"}
                ]
            }
            return f"[INTERACTIVE_TASK] {json.dumps(fallback, ensure_ascii=False)}"
...
Yêu cầu:
1. Gồm đúng 5 câu hỏi.
2. KHÔNG đưa đáp án trước.
3. KHÔNG sử dụng LaTeX.
4. BẮT BUỘC TRẢ VỀ ĐÚNG ĐỊNH DẠNG JSON SAU, không có markdown:
{{
  "type": "practice",
  "topic": "{topic}",
...
"""
        try:
            response = await self.llm_service.generate_response(
                system_prompt="You are a Math teacher. Output ONLY valid JSON representing the practice exercises. DO NOT wrap in ```json.",
                user_prompt=prompt
            )
            response = response.replace("```json", "").replace("```", "").strip()
            data = json.loads(response)
            ...
            fallback = {
                ...
                "topic": topic,
                "questions": [
                    {"id": 1, "type": "fill_blank", "question": "Tính 5 + 5 = ?"},
                    {"id": 2, "type": "fill_blank", "question": "Điền số: 10 - ... = 2"},
                    {"id": 3, "type": "radio", "question": "3 + 3 = ?", "options": ["5", "6", "7"]}
                ]
            }
            return f"[INTERACTIVE_TASK] {json.dumps(fallback, ensure_ascii=False)}"
